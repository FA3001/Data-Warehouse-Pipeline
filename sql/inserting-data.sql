...
select * from issue_dimension LIMIT 5;
...
select * from resolution_dimension LIMIT 5;
-------------------------------
...
FROM
    complaints cc
JOIN date_dimension dd ON dd.year = EXTRACT(YEAR FROM cc."Date sent to company")
                      AND dd.month = EXTRACT(MONTH FROM cc."Date sent to company")
                      AND dd.day = EXTRACT(DAY FROM cc."Date sent to company")
